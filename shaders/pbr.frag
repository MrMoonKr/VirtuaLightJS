...
void main( void ) 
{
    ...
    int nbLights        = int( u_perPass.nbLights ) ;   // number of lights
    ...
    {
        if ( distance( pos, u_perPass.lights[i].position ) <= MAX_DIST ) // 라이트에 너무 근접하면
        {
            color = getLightColor( u_perPass.lights[i], pos );
            return;
        }

        vec3 incidentVector = normalize( u_perPass.lights[i].position - pos ) ; // L
        float directionnalAttenuation = max( dot( normal, incidentVector ), 0.0 ) ; // NdotL
...
{
    // assume normal, 
    // the interpolated vertex normal and 
...
// Bi-Linear interpolation is implemented in this function with the help of nearest four data.
vec4 tex2DBiLinear( sampler2D textureSampler_i, vec2 texCoord_i )
{
